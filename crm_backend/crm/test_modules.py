...
class ModelTestCase(TestCase):
    ...
    def test_promotion_usage_limits(self):
        ...
        promotion = Promotion.objects.create(name="Test Promotion", description="Test promotion description", type="Coupon", start_date=timezone.now(), end_date=timezone.now() + timezone.timedelta(days=7), discount_type="Percentage", discount_value=10, usage_limits=2)
        self.assertEqual(promotion.expiration_date, None)
        promotion.usage_limits = 0
        promotion.save()
    ...
    def test_loyalty_model_upgrade_tier(self):
        ...
        customer = Customer.objects.create(first_name="John", last_name="Doe", email="john@example.com", phone_number="1234567890", address="123 Main St", city="Anytown", state="Somestate", country="USA", postal_code="12345")
        loyalty_model = LoyaltyModel.objects.create(customer=customer, points=200, tier='Bronze')
        LoyaltyModel.upgrade_tier(loyalty_model.id)
